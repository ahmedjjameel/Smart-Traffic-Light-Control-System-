...
x = {'right':[0,0,0], 'down':[605,657,627], 'left':[1400,1400,1400], 'up':[758,697,727]}    
y = {'right':[498,440,468], 'down':[0,0,0], 'left':[348,396,366], 'up':[800,800,800]}
...
mid = {'right': {'x':705, 'y':400}, 'down': {'x':695, 'y':450}, 'left': {'x':695, 'y':425}, 'up': {'x':695, 'y':400}}
...
class Vehicle(pygame.sprite.Sprite):
    ...
    def move(self):
        if(self.direction=='right'):
            ...
            if(self.willTurn==1):
                if(self.lane == 2):
                    if(self.crossed==0 or self.x+self.image.get_rect().width<stopLines[self.direction]+60):     #+40
                        ...
                        if(self.turned==0):
                            ...
                            self.image = pygame.transform.rotate(self.originalImage, -self.rotateAngle)      #  self.rotateAngle
                            self.x += 1.2   #2.0   #+2.4
                            self.y += 1.8   #2.4   #-2.8
                            ...
                            if(self.crossedIndex==0 or (self.y<(vehiclesTurned[self.direction][self.lane][self.crossedIndex-1].y + vehiclesTurned[self.direction][self.lane][self.crossedIndex-1].image.get_rect().height + movingGap))):
                                self.y += self.speed    # -= self.speed     self.y>
                elif(self.lane == 1):
                    if(self.crossed==0 or self.x+self.image.get_rect().width<mid[self.direction]['x']):     # <mid
                        if((self.x+self.image.get_rect().width<=self.stop or (currentGreen==0 and currentYellow==0) or self.crossed==1) and (self.index==0 or self.x+self.image.get_rect().width<(vehicles[self.direction][self.lane][self.index-1].x - movingGap) or vehicles[self.direction][self.lane][self.index-1].turned==1)):                 
                            self.x += self.speed
                    else:
                        if(self.turned==0):
            ...
            if(self.willTurn==1):
                if(self.lane == 2):
                    if(self.crossed==0 or self.y+self.image.get_rect().height<stopLines[self.direction]+60):    #+50
            ...
            if(self.willTurn==1):
                if(self.lane == 2):
                    if(self.crossed==0 or self.x>stopLines[self.direction]-60):     # -70
